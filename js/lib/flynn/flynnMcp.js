...
(function () {
    ...
    Flynn.Mcp = Class.extend({
        ...
        init: function (
            ...
            this.optionQuiethalt = Flynn.Util.getUrlFlag('quiethalt');
            ...
            // SUPPORT: iOS
            this.browserIsIos = (!!navigator.userAgent.match(/(iPad|iPhone|iPod)/g));
            ...
            // Set Vector mode
            let vectorMode = null;
            const vectorModeFromUrl = Flynn.Util.getUrlValue('vector');
            for (const key in Flynn.VectorMode) {
            ...
            // Get the dimensions of the viewport
                ...
                // Determine game size
                const targetWidth = self.canvasWidth;
                const targetHeight = self.canvasHeight;
                const multiplier = Math.min((viewport.height / targetHeight), (viewport.width / targetWidth));
                ...
                const top = Math.floor(viewport.height / 2 - actualCanvasHeight / 2);
                const left = Math.floor(viewport.width / 2 - actualCanvasWidth / 2);
        ...
        devHalt: function () {
            ...
            const box_height = 60;
            const box_margin = 20;
            const box_width = this.canvasWidth - box_margin * 2;
            const box_x = box_margin;
            const box_y = this.canvasHeight / 2 - box_height / 2;

            this.halted = true;
            console.log('DEV: Execution halted');
            if (!this.optionQuiethalt) {
        ...
        run: function () {
            ...
            this.canvas.animate(function (elapsed_ticks) {
            // Change state (if pending)
                ...
                self.timers.update(elapsed_ticks);

                // Process state (if set)
                if (self.current_state) {
                    self.current_state.handleInputs(self.input, elapsed_ticks);
                    self.current_state.update(elapsed_ticks);
