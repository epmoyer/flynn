(function () {
    ...
    Flynn.OptionDescriptor = Class.extend({
        ...
        currentPromptValueIndex: function () {
            for (let i = 0, len = this.promptValues.length; i < len; i++) {
    ...
    Flynn.OptionManager = Class.extend({
        ...
        addOption: function (keyName, type, defaultValue, currentValue, promptText, promptValues, commandHandler) {
            const descriptor = new Flynn.OptionDescriptor(keyName, type, defaultValue, currentValue, promptText, promptValues, commandHandler);
        ...
        addOptionFromVirtualButton: function (virtualButtonName) {
            const keyCode = Flynn.mcp.input.getVirtualButtonBoundKeyCode(virtualButtonName);
            const keyName = virtualButtonName;
            const descriptor = new Flynn.OptionDescriptor(keyName, Flynn.OptionType.INPUT_KEY, keyCode, keyCode, keyName, null, null);
            this.optionDescriptors[keyName] = descriptor;
        },

        setOption: function (keyName, value) {
            if (keyName in this.optionDescriptors) {
                const optionDescriptor = this.optionDescriptors[keyName];
        ...
        revertToDefaults: function () {
            for (const keyName in this.optionDescriptors) {
                const descriptor = this.optionDescriptors[keyName];
        ...
        // Revert to defaults
            ...
            const optionKeyNames = this.getOptionKeyNames();
            for (let i = 0, len = optionKeyNames.length; i < len; i++) {
                const optionKey = optionKeyNames[i];
                const cookieKey = document.title + ':OPT_' + optionKey;
                const attributeValue = Cookies.get(cookieKey);
                if (attributeValue) {
        ...
        // All option types except COMMAND (i.e. INPUT_KEY, MULTI, and BOOLEAN) will be saved to cookies.
            if (this.optionDescriptors[optionKey].type != Flynn.OptionType.COMMAND) {
                const cookieKey = document.title + ':OPT_' + optionKey;
                const optionValue = this.getOption(optionKey);
                const cookieValue = JSON.stringify(optionValue);
        ...
        // Get all option key names and return them ordered by input keys, multi options, booleans, and commands.
            ...
            const keyNamesInputKey = [];
            const keyNamesCommand = [];
            const keyNamesMulti = [];
            const keyNamesBoolean = [];
