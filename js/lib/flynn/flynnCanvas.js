...
(function () {
    ...
    Flynn.Canvas = Class.extend({
        ...
        init: function (width, height, canvas, background_color) {
            ...
            const gauge_x = width - 131;
            const gauge_spacing = 8;
            ...
            // NOTE: The "Context" (ctx) used in Flynn is (for legacy reasons) NOT
                ...
                ctx.drawFpsGague = function (position, color, percentage) {
                    ...
                    const draw_pos = position.clone().addScalar(0.5);
                    this.beginPath();
                    const length = 60;
                    const height = 6;
                    const x_needle = percentage * length;
                ...
                //
                    ...
                    const line_color = Flynn.Util.shadeColor(color, config.lineBrightness);
                ...
                ctx.vectorLineTo = function (x, y) {
                    ...
                    if (this.is_world) {
                    // World coordinates
                        const world = this.worldToScreen(x, y, true);
                        x = world.x;
                        y = world.y;
                    }
                ...
                ctx.vectorMoveTo = function (x, y) {
                    ...
                    if (this.is_world) {
                    // World coordinates
                        const world = this.worldToScreen(x, y, false);
                        x = world.x;
                        y = world.y;
                    }
                    if (this.index_vector_vertex < ctx.MAX_VERTICIES_x2) {
                ...
                //
                    ...
                    let i, len, j, len2, character, polygon, pen_up;
                    let string_x = opts.x;
                    let string_y = opts.y;
                    let character_x = 0;
                    let character_y = 0;

                    const step = opts.scale * opts.font.CharacterSpacing * opts.spacing;
                    ...
                    // ----------------------
                        ...
                        const unit_x_v = new Victor(1, 0).rotate(opts.angle);
                        const unit_y_v = new Victor(0, 1).rotate(opts.angle);
                ...
                //       for no stretching.
                    ...
                    let render_angle = opts.angle;
                    let render_angle_step = Math.asin(opts.font.CharacterSpacing * opts.scale / opts.radius);

                    if (opts.stretch) {
                        render_angle_step *= 1.4;
                    }
                    let renderAngleOffset = 0;
                    if (opts.is_centered) {
                    ...
                    let character_angle = render_angle + Math.PI / 2;
                    if (opts.is_reversed) {
                    ...
                    for (let i = 0, len = text.length; i < len; i++) {
                        ...
                        const polygon = this.charToPolygon(character, opts.font);

                        // Render character
                        let pen_up = false;
                        for (let j = 0, len2 = polygon.length; j < len2; j += 2) {
                            if (polygon[j] == Flynn.PEN_COMMAND) {
                                pen_up = true;
                            } else {
                                if (opts.stretch) {
                                    ...
                                    let character_x = polygon[j + 1];
                                    if (!opts.is_reversed) {
                                        character_x = opts.font.CharacterHeight - character_x;
                                    }
                                    const x_log = Flynn.Util.logish(
                                        character_x,
                                        0, // min
                                    ...
                                    const draw_radius = opts.radius + (x_log - opts.font.CharacterHeight / 2) * opts.scale * opts.stretch;
                                    const draw_angle = render_angle +
                                    ...
                                    const x = polygon[j] - opts.font.CharacterWidth / 2;
                                    const y = polygon[j + 1] - opts.font.CharacterHeight / 2;
                                    const c = Math.cos(character_angle);
                                    const s = Math.sin(character_angle);
        ...
        animate: function (animation_callback_f) {
            ...
            const self = this;
            let callback_f = function (timeStamp) {
                // ---------------------------
                // Calculate FPS and pacing
                // ---------------------------
                let timeNow;
                let performance_proxy;
                ...
                const deltaMsec = timeNow - self.previousTimestamp;
                ...
                let elapsed_ticks = (60 * (timeNow - self.previousTimestamp)) / 1000;
