(function () { "use strict";
  Flynn.Text = Class.extend({
    ...
    updatePoints: function() {
      var ctx = Flynn.mcp.canvas.ctx;
      var step = this.scale * this.font.CharacterSpacing * this.spacing;
    ...
    createNonRotatedPoints: function(ctx, step) {
      var i, len, j, len2, character, polygon;
      var character_x = 0;
      var character_y = 0;
    ...
    createRotatedPoints: function(ctx, step) {
      ...
      var unit_x_v = new Victor(1, 0).rotate(this.angle);
      var unit_y_v = new Victor(0, 1).rotate(this.angle);
    ...
    render: function(ctx) {
      ...
      var penUp = false;
      for (var i = 0; i < this.pointsLength; i += 2) {
        var x = this.points[i];
