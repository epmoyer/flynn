...
(function () {
    ...
    Flynn.StateEnd = Flynn.State.extend({
        ...
        init: function (score, leaderboard, color, title, prompt, parentState, score_is_seconds, font_aspect_ratio, game_version) {
            ...
            this.nickname = '';
            const worstEntry = this.leaderboard.getWorstEntry();
            ...
            const reference_width = 12 * this.REFERENCE_KEY_SIZE + 10 * this.REFERENCE_KEY_GAP;
            const fill_width = Flynn.mcp.canvasWidth - 2 * this.KEYBOARD_MARGIN;
            const key_scale = fill_width / reference_width;
            const num_rows = this.TOUCH_KEYBOARD_ROWS.length;
            const reference_height = (
                num_rows * this.REFERENCE_KEY_SIZE +
            (num_rows - 1) * this.REFERENCE_KEY_GAP
            );
            const keyboard_height = reference_height * key_scale;
        ...
        getAndCleanNickname: function () {
            const text = Flynn.mcp.input.getTextCapture();
        ...
        handleInputs: function (input, elapsed_ticks) {
            ...
            if (Flynn.mcp.browserSupportsTouch) {
                let touched_key = null;
                let distance_touched_key = 100000000000;
                const touch_location_v = input.getLastTouchLocation();
                if (touch_location_v !== null) {
                    let card_center_v, distance;
                    const self = this;
                    ...
                    if (touched_key !== null) {
                        let key_lookup = touched_key.text.toLowerCase();
                        switch (key_lookup) {
                        case 'back':
                            key_lookup = 'delete';
                            break;
        ...
        render: function (ctx) {
            ...
            // -------------------------
                ...
                for (let i = 0, len = this.leaderboard.leaderList.length; i < len; i++) {
                    const leader = this.leaderboard.leaderList[i];
                    ctx.vectorText2({
                        text: leader.name,
                        scale: 2,
                ...
                const text = Flynn.mcp.browserSupportsTouch
