(function () {
    ...
    Flynn.InputHandler = Class.extend({
        ...
        init: function () {
            ...
            this.keyDownHandler = function (evt) {
               ...
               evt.keyCode == Flynn.KeyboardMap.tab) {
                ...
                if (self.iCadeModeEnabled) {
                    let index = self.iCade.keyDownCodes.indexOf(evt.keyCode);
                    let newEvt;
                    if (index > -1) {
                    ...
                    document.addEventListener(
                        ...
                        // event.preventDefault();
                            ...
                            if (Flynn.mcp.mousetouchEnabled) {
                                const canvas = Flynn.mcp.touch_control_canvas.canvas;
                                ...
                                const event_caught = self.handleTouchStart(x, y, self.MOUSE_IDENTIFIER);
                                if (!event_caught) {
                ...
                try {
                    document.addEventListener(
                        ...
                        // event.preventDefault();
                            ...
                            const event_caught = self.handleTouchEnd(x, y, self.MOUSE_IDENTIFIER);
                            if (!event_caught) {
            ...
            try {
                document.addEventListener(
                    ...
                    function (event) {
                        let i;
                        let event_caught = false;
                        for (i = 0; i < event.changedTouches.length; i++) {
                            const touch = event.changedTouches[i];

                            const canvas = Flynn.mcp.touch_control_canvas.canvas;
                            const rect = canvas.getBoundingClientRect();
                            const x = (touch.pageX - rect.left) * canvas.width / canvas.clientWidth;
                            const y = (touch.pageY - rect.top) * canvas.height / canvas.clientHeight;
        ...
        // Returns true if a control caught the event, else false.
            ...
            let name, region, joystick, touched, event_caught;
        ...
        // Returns true if a control caught the event, else false.
            ...
            let name, region, joystick, direction, i, event_caught;
        ...
        getConfigurableVirtualButtonNames: function () {
            const names = [];
            for (const name in this.virtualButtons) {
        ...
        keyCodeToKeyName: function (keyCode) {
            for (const keyName in Flynn.KeyboardMap) {
        ...
        unbindVirtualButtonFromKey: function (name) {
            ...
            // Remove binding from keyCodeToVirtualButtonName
                for (const keyCode in this.keyCodeToVirtualButtonName) {
        ...
        // Touch regions can be bound to virtual buttons which are also bound to keys.
            ...
            const touchRegion = new Flynn.TouchRegion(name, left, top, right, bottom, shape, visible_states);
        ...
        addVirtualJoystick: function (opts) {
            opts = opts || {};
            const joystick = new Flynn.VirtualJoystick(opts);
