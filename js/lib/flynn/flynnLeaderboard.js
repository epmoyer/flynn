(function () {
    ...
    Flynn.Leaderboard = Class.extend({
        ...
        isEligible: function (score) {
            ...
            const worst_entry = this.getWorstEntry();
            const better_than_worst = (
                (!this.sortDescending && score < worst_entry.score) ||
            (this.sortDescending && score > worst_entry.score));
            const leaderboard_full = this.leaderList.length == this.maxItems;
        ...
        loadFromCookies: function () {
            ...
            ':LeaderBoard';
            const json_text = Cookies.get(key);
            if (json_text) {
        ...
        saveToCookies: function () {
            ...
            (Flynn.mcp.developerModeEnabled ? '_Develop' : '') +
            ':LeaderBoard';
            const value = JSON.stringify(this.leaderList);
        ...
        // sort high score in ascending order
            ...
            // Drop the last
            const extraItems = this.leaderList.length - this.maxItems;
            if (extraItems > 0) {
